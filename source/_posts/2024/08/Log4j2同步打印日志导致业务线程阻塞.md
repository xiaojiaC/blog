...
public Class<?> loadClass(String name, boolean resolve) throws ClassNotFoundException {
    synchronized (JreCompat.isGraalAvailable() ? this : getClassLoadingLock(name)) { // 这里有并行类加载同步锁
        ...
        // (2) 在我们的本地存储库中尝试找指定的类（其实是调SecurityManager.checkPackageDefinition来查找，不涉及忽略）
        if (log.isDebugEnabled()) {
...
结合上述 Log4j2 日志配置和调用栈，得知打印我们日志使用的是`PatternLayout`，其内配置了`%xwEx`模式字符串，触发了`ExtendedThrowablePatternConverter`转换器解析异常栈信息，在转换中会获取每个调用栈的`Class`信息，从而诱发相关类的加载。
...
1. 检查我们之前加载的本地类缓存（该类已加载过的类缓存`resourceEntries` Map，未加载过此时没有）；
...
5. 在我们的本地存储库中尝试找指定的类（其实是调`SecurityManager.checkPackageDefinition`来查找，不涉及忽略）；
...
由于应用依赖大量上游 RPC 调用，我们知道像[DUBBO](https://cn.dubbo.apache.org/zh-cn/#/docs/user/quick-start.md?lang=zh-cn)类似的微服务框架，内部其实是组装好消息对象（包含接口、方法名、别名、参数类型、参数值等）调用目标机，反射执行远程方法调用。我们的服务 Provider 接收到消息后，并行请求 30 多个上游获取权益，其中任何一个调用异常会打印异常栈，继而诱发`GeneratedMethodAccessorXxx`类加载。
...
Log4j2 中记录日志的方式有同步日志和异步日志两种方式，其中异步日志又可分为使用`AsyncAppender`和使用`AsyncLogger`两种方式。使用 Log4j2 有三种日志模式：全异步日志、混合模式、同步日志，[性能](https://logging.apache.org/log4j/2.x/performance.html)从高到底，线程越多效率越高，也可以避免日志卡死线程情况发生。
...
也可以在`src/java/resources`目录添加`log4j2.system.properties`配置文件：
...
- 建议使用`RandomAccessFileAppender`, 并且设置`immediateFlush="false"`，但注意设置后`tail -f`将不能实时观察最新日志。

> [Log4j2 Architecture](https://logging.apache.org/log4j/2.x/manual/architecture.html) > [Log4j2 Async](https://logging.apache.org/log4j/2.x/manual/async.html)
